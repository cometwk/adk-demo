...
export function reconcileDiagnostic(systemVerdict: DiagnosticVerdict, modelVerdict: DiagnosticVerdict): Reconciliation {
...
function diagnoseConflict(sys: SystemVerdict_Predictive, model: ModelVerdict_Predictive): ReconciliationLikelyCause {
...
function buildConflictExplanation(
  ...
  const sysLabel =
    sys.ranking.find((r) => r.candidateId === sys.recommendedCandidateId)?.label ?? sys.recommendedCandidateId
