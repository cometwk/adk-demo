...
export async function runAgent(goal: string, graph: Graph, history: ModelMessage[] = []) {
  ...
  const messages = [...history, { role: 'user', content: goal } satisfies ModelMessage]
  ...
  const result = await generateText({
    ...
    onStepFinish({ stepNumber, text, staticToolCalls, staticToolResults, finishReason }) {
