...
function butForScore(causeEventId: string | undefined, outcomeEventId: string, eventStore: EventStore): number {
...
function sufficiencyScore(cause: CandidateCause, causalGraph: CausalGraph): number {
...
function pathCompletenessScore(cause: CandidateCause, eventStore: EventStore, outcomeOccurredAt: string): number {
...
function temporalPlausibilityScore(cause: CandidateCause, outcomeOccurredAt: string, eventStore: EventStore): number {
...
export function scoreCauses(input: AttributionInput): AttributionResult[] {
  const {
    ...
    eventStore,
    causalGraph,
    weights = DEFAULT_ATTRIBUTION_WEIGHTS,
  } = input
  ...
  const results: AttributionResult[] = causes.map((cause) => {
    ...
    const causeEventId = eventStore
      ...
      .find(
        (e) => e.type === cause.causalPathRef.rootCauseMatcher || e.type.includes(cause.causalPathRef.rootCauseMatcher)
      )?.id
...
export function isOverdetermined(results: AttributionResult[], threshold = 0.4): boolean {
