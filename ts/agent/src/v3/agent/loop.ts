	...
	for (let step = 0; step < MAX_STEPS; step++) {
		...
		if (!validation.valid) {
			...
			if (consecutiveErrors > MAX_CONSECUTIVE_ERRORS) {
				console.log(
					`❌ Aborted after ${MAX_CONSECUTIVE_ERRORS} consecutive errors:`,
					validation.error,
				);
				break;
			}
			console.log(
				`⚠️  Validation error (${consecutiveErrors}/${MAX_CONSECUTIVE_ERRORS}):`,
				validation.error,
			);
		...
		if (!obs.success) {
			...
			console.log(
				`⚠️  Execution error (${consecutiveErrors}/${MAX_CONSECUTIVE_ERRORS}):`,
				obs.error,
			);
