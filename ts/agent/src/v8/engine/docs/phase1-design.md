...
interface RuntimeOrchestrator {
  ...
  executeGraphQuery(query: GraphQuery): Promise<ToolResult<GraphQueryResult>>
...
class SemanticRuntimeOrchestrator implements RuntimeOrchestrator {
  ...
  async executeGraphQuery(query: GraphQuery): Promise<ToolResult<GraphQueryResult>> {
    // 1. Cache check
    if (this.cache) {
    ...
    // 2. Policy validation (traversal depth, etc.)
    this.validateTraversalPolicy(query)

    // 3. Execute via GraphStore
  ...
  async executeComputeQuery(query: ComputeQuery): Promise<ToolResult<ComputeQueryResult>> {
    // 1. Validate no graph traversal
    if (query.requiresTraversal) {
      return toolErr('INVALID_ARGS', 'Compute query should not include traversal')
    }

    // 2. Execute via ComputeStore
    const result = await this.computeStore.aggregate(query)

    // 3. Inject into FactStore
    this.injectFacts(result)

    return result
  }

  private validateTraversalPolicy(query: GraphQuery): void {
    const depth = query.traverse?.length ?? 0
    if (depth > MAX_TRAVERSAL_DEPTH) {
      throw new Error(`Traversal depth ${depth} exceeds limit ${MAX_TRAVERSAL_DEPTH}`)
    }
  }

  private injectFacts(result: GraphQueryResult | ComputeQueryResult): void {
    // 将结果中的关键数据注入 FactStore
    // 供后续推理使用
  }
}
```
...
3. 不要用 graph_query 做 global aggregation（效率极低）

## 执行顺序
4. 先用 graph_query 缩小候选集合
5. 再用 compute_query 在候选集上做聚合
6. 避免全表扫描

## 事实收集
7. 每次读取数据后，立即 bind_fact
8. 中间结果自动进入 FactStore
```
...
```text
Step 1: Agent 调用 graph_query
  → 找出所有 Agent → binds_merch → Merch
  → 得到候选商户集合 [M001, M002, ...]

Step 2: Agent 调用 compute_query
  → source: OrderDaily
  → filters: merch_no in [M001, M002, ...]
  → metrics: count(*)
  → groupBy: merch_no
  → 得到各商户本月交易数

Step 3: Agent 观察 compute_query 结果
  → 发现 M003, M005 的 count = 0

Step 4: Agent Replan
  → 继续探索这些商户的属性（是否新进件？）
  → 或者直接给出答案

Step 5: Agent 输出 verdict
```
...
class FactStore {
  ...
  // 新增：绑定中间结果
  bind(binding: FactBinding): void
  bindFromQueryResult(result: GraphQueryResult | ComputeQueryResult): void
}
```

### 8.3 Runtime 自动注入

Runtime Orchestrator 在执行查询后，自动将关键数据注入 FactStore：

```typescript
class SemanticRuntimeOrchestrator {
  private injectFacts(result: GraphQueryResult): void {
    // 将 nodeId 集合注入为候选
    const nodeIds = result.rows.map(r => r.nodeId)
    this.factStore.bind({
      entityId: 'workspace',
      property: 'candidates',
      value: nodeIds,
      source: { kind: 'graph_query', ref: '...' },
      confidence: 0.9,
...
│   ├── cache-store.ts         — CacheStore 接口（可选）
...
| P1  | FactStore 简化        | 保持 V6，增加自动注入           |
| P1  | Agent Executor      | 适配新 tools              |
| P2  | VectorStore 接口      | 接口定义，内存 stub 实现        |
| P2  | vector_query 工具     | Phase 1 可选             |
| P2  | CacheStore          | Phase 1 可选             |
...
const GraphTraversalQuerySchema = z.object({
  match: MatchClauseSchema,
  traverse: z.array(TraverseStepSchema).optional(),
  return: ReturnClauseSchema.optional(),
})
