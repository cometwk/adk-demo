...
export class DataModel extends BaseNode {
	@agentProperty({ returns: "number", description: "数据 SLA 刷新时间窗口（小时），超过则视为过时" })
	freshnessSlaHours: number;

	@agentProperty({ returns: "number", description: "测试覆盖率 0-1，低于 0.6 为质量风险" })
	testCoverage: number;
	...
	assessFreshnessRisk(args: { hoursSinceLastRefresh: number }): { stale: boolean; overdueFactor: number } {
...
export class DataSource extends BaseNode {
	@agentProperty({ returns: "string", description: "数据源类型：api | database | streaming" })
	...
	checkReliability(args: { currentDelayHours: number }): { reliable: boolean; delayRatio: number } {
...
export class Dashboard extends BaseNode {
	@agentProperty({ returns: "'low' | 'medium' | 'high' | 'critical'", description: "业务关键度" })
	criticalityLevel: "low" | "medium" | "high" | "critical";

	@agentProperty({ returns: "number", description: "直接依赖的 DataModel 数量" })
