...
export function buildLibraryCausalGraph(): CausalGraph {
	const edges: CausalEdge[] = [
		...
		},
		{
			...
			cause: { kind: "fact_condition", matcher: "Book.isNew=true && Book.daysSinceShelved<7" },
		...
		},
		{
			...
			cause: { kind: "fact_condition", matcher: "Book.status=borrowed || Book.canCheckout=false" },
			effect: { kind: "event_type", matcher: "borrow_rejected" },
