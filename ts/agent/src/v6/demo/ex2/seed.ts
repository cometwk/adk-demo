...
export function seedDbtGraph(): Graph {
  ...
  const ordersDaily = new DataModel('orders_daily', 24, 0.7, true, 30_000) // 24h SLA
  const revenueSummary = new DataModel('revenue_summary', 24, 0.45, true, 5_000) // low test coverage!
  const customerSegments = new DataModel('customer_segments', 48, 0.8, false, 10_000) // no owner!
...
export function seedDbtFactStore(): FactStore {
  ...
  const bindings: FactBinding[] = [
    ...
    {
      entityId: 'orders_raw',
      property: 'hoursSinceLastRefresh',
      value: 1.5,
      source: { kind: 'aggregation' },
      confidence: 0.95,
      validFrom: now,
      observedAt: now,
    },
    ...
    {
      entityId: 'orders_daily',
      property: 'downstreamDashboardCount',
      value: 2,
      source: { kind: 'aggregation' },
      confidence: 1.0,
      validFrom: now,
    ...
    {
      entityId: 'src_orders_api',
      property: 'avgUpdateIntervalHours',
      value: 1,
      source: { kind: 'graph_property' },
      confidence: 1.0,
      validFrom: now,
