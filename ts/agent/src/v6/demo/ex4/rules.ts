...
export function registerLibraryRules(): void {
	...
	registerRule({
		...
		evaluator(ctx) {
			...
			if (days === undefined) {
				return { triggered: false, missingFacts: [{ entityId, property: "daysOnShelf" }] };
			}
	...
	registerRule({
		...
		evaluator(ctx) {
			...
			if (hasOverdue === undefined) {
				return { triggered: false, missingFacts: [{ entityId, property: "hasOverdueBook" }] };
			}
