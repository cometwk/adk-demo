...
async function main(round?: 'round1' | 'round2') {
  ...
  if (round === undefined || round === 'round1') {
    ...
    if (predictiveResult.systemVerdict?.mode === 'predictive') {
      ...
      if (sv.ranking.length > 0) {
        ...
        for (const r of sv.ranking) {
          ...
          console.log(
            `    ${r.label}: score=${r.normalizedScore.toFixed(3)}, confidence=${r.confidence.toFixed(2)}${blocked}`
          )
        }
      }
    }
    ...
    for (const ev of predictiveResult.evidence.slice(0, 6)) {
      console.log(`  [${ev.sourceKind}] entities=${ev.entityIds.join(',')} — ${ev.content.slice(0, 100)}`)
    }
  ...
  if (round === undefined || round === 'round2') {
    ...
    const {
      ...
      eventStore: es2,
      causalGraph: cg2,
    } = setupLibraryScenario({ relations: libraryOntology.relations })
