...
function getData(result: ReturnType<GraphQueryEngine['execute']>) {
  if (!result.ok) throw new Error(`query failed: [${result.code}] ${result.message}`)
...
describe('graph_query: MATCH only', () => {
  it('返回全部 4 个 Reader', () => {
    ...
    const d = getData(engine.execute(q))
  ...
  it('按 membershipLevel=gold 过滤 Reader', () => {
    const q: GraphQuery = {
  ...
  it('按 currentBorrowCount=3 过滤 Reader（老王借满）', () => {
    const q: GraphQuery = {
  ...
  it('找所有新书（daysOnShelf < 7）：book_hp3 和 book_quantum', () => {
    const q: GraphQuery = {
  ...
  it('找无库存书（availableCopies=0）：book_tb2 和 book_hp2', () => {
    const q: GraphQuery = {
  ...
  it('membershipLevel in [gold, silver] 过滤', () => {
    const q: GraphQuery = {
      match: {
        type: 'Reader',
...
describe('graph_query: TRAVERSE 单跳', () => {
  ...
  it('lao_wang 借满 3 本：hp1、sapiens、tb1', () => {
    const q: GraphQuery = {
  ...
  it('找注册在 branch_west 的 Reader（require=exists + where 分馆名）', () => {
    const q: GraphQuery = {
...
describe('graph_query: require=exists / none', () => {
  it('require=exists：有逾期书的 Reader（只有 xiao_li）', () => {
    const q: GraphQuery = {
      match: { type: 'Reader', alias: 'reader' },
  ...
  it('require=none：没有逾期书的 Reader（3 个）', () => {
    const q: GraphQuery = {
      match: { type: 'Reader', alias: 'reader' },
  ...
  it('require=exists + where：借了科学类书（daysOnShelf >= 50）的 Reader', () => {
    const q: GraphQuery = {
      match: { type: 'Reader', alias: 'reader' },
      traverse: [
        {
...
describe('graph_query: TRAVERSE 多跳', () => {
  ...
  it('2 跳：branch_central 提供的书所属类目', () => {
  ...
  it('3 跳：Series → part_of(in) → Book → borrows(in) → Reader', () => {
    // 找借阅过三体系列书的读者
    const q: GraphQuery = {
...
describe('graph_query: RETURN fields projection', () => {
  it('只返回 Reader 的 name 和 membershipLevel', () => {
    const q: GraphQuery = {
  ...
  it('limit/offset 分页', () => {
    ...
    const d1 = getData(engine.execute(q1))
    const d2 = getData(engine.execute(q2))
...
describe('graph_query: aggregate', () => {
  ...
  it('avg daysOnShelf of Book ≈ 105.2', () => {
  ...
  it('sum availableCopies of Book = 12', () => {
  ...
  it('groupBy isRestricted：限制类目 1 个，非限制 2 个', () => {
    const q: GraphQuery = {
      match: { type: 'Category' },
      return: {
        aggregate: {
  ...
  it('2 跳 + 聚合：刘慈欣所有书的 availableCopies 之和', () => {
...
describe('graph_query: error handling', () => {
  it('TRAVERSE from 引用不存在的 alias → INVALID_ARGS', () => {
    const q: GraphQuery = {
      match: { type: 'Reader', alias: 'reader' },
  ...
  it('RETURN alias 不存在 → INVALID_ARGS', () => {
    const q: GraphQuery = {
      match: { type: 'Reader' },
