...
export function createCounterfactualTools(policy: PolicyContext) {
  const simulate = tool({
    ...
    inputSchema: z.object({
      mode: z.enum(['what_if', 'but_for']).describe('what_if = 正向模拟；but_for = 擦除过往事件'),
      ...
      eraseEventId: z.string().optional().describe('but_for 模式下要从 EventStore 中擦除的事件 ID'),
      impactPreview: z
        .object({
          before: z.string(),
    ...
    execute: async ({ mode, description, overrides, eraseEventId, impactPreview }): Promise<ToolResult> => {
