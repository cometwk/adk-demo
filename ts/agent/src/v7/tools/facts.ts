...
export function createFactTools(policy: PolicyContext) {
  const bind_fact = tool({
    ...
    inputSchema: z.object({
      ...
      confidence: z.number().min(0).max(1).default(0.9).describe('置信度 0..1；直接读取默认为 0.9'),
      validFrom: z.string().optional().describe('ISO 8601: 该值开始生效的时间（默认为当前时间）'),
  ...
  const aggregate_facts = tool({
    description:
      '对多个实体的属性绑定事实进行聚合计算。支持操作：sum, avg, count, min, max。' + '聚合后使用 bind_fact 绑定结果。',
    inputSchema: z.object({
      entityIds: z.array(z.string()).describe('要聚合的实体 ID'),
      property: z.string().describe('要聚合的属性'),
      operation: z.enum(['sum', 'avg', 'count', 'min', 'max']).describe('聚合操作'),
    }),
