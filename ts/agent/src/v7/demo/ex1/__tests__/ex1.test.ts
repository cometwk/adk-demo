...
import { burnoutFixtures, capacityFixtures, emptyGraph, teamLoadFixtures } from '../ruleFixtures'
import { seedEventStore, seedFactStore, setupScenario } from '../seed'
...
describe('engineer_burnout_threshold', () => {
  it('alice (senior, 85h) → triggered', () => {
    ...
    const result = evaluateSingleRule(
      'engineer_burnout_threshold',
      burnoutFixtures.alice_senior_burnout,
      graph,
      'alice'
    )
  ...
  it('alice and bob evaluated independently — no missingFacts', () => {
    const { graph } = setupScenario()
    const aliceResult = evaluateSingleRule(
      'engineer_burnout_threshold',
      burnoutFixtures.alice_senior_burnout,
      graph,
      'alice'
...
describe('team_capacity_overload', () => {
  ...
  it('normal team → NOT triggered', () => {
    ...
    const result = evaluateSingleRule(
      ...
      capacityFixtures.frontend_normal,
      graph,
      'team_frontend'
    )
    expect(result?.result.triggered).toBe(false)
...
describe('MCDA scoreCandidates', () => {
  ...
  it('veto LOW when hard_constraint triggers', () => {
    ...
    const overloadResult = evaluateSingleRule(
      'team_capacity_overload',
      capacityFixtures.overloaded_team,
      graph,
      'team_x'
    )
    ...
    const candidates: CandidateAnswer[] = [
      {
        id: 'c1',
        label: 'HIGH',
        description: 'High delivery risk',
        supportingEvidenceIds: [],
      },
      {
        id: 'c2',
        label: 'MEDIUM',
        description: 'Medium delivery risk',
        supportingEvidenceIds: [],
      },
      {
        id: 'c3',
        label: 'LOW',
        description: 'Low delivery risk',
        supportingEvidenceIds: [],
      },
    ]
...
describe('reconcilePredictive', () => {
  ...
  it('conflict when picks differ → surfacedToUser = true, likelyCause set', () => {
    const systemV: SystemVerdict_Predictive = {
      ...
      ranking: [
        ...
        },
        {
          ...
          normalizedScore: 1.0,
          confidence: 0.9,
          triggeredRuleIds: ['engineer_burnout_threshold'],
          blockingRuleIds: [],
          rationale: '',
        },
      ],
      recommendedCandidateId: 'c3',
      confidence: 0.4,
      vetoedLabels: [],
      notes: [],
    }
    const modelV: ModelVerdict_Predictive = {
      ...
      recommendedCandidateId: 'c1',
      confidence: 0.85,
      rationale: 'Alice is burning out',
      citedEvidenceIds: [],
      citedRuleIds: ['engineer_burnout_threshold'],
    }
    const reconciliation = reconcilePredictive(systemV, modelV)
