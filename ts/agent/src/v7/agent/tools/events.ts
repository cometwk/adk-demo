...
export function createEventTools(
  ...
  const query_events = tool({
    ...
    inputSchema: z.object({
      ...
      eventTypes: z.array(z.string()).optional().describe('按事件类型字符串过滤'),
    }),
  ...
  const walk_causal_graph = tool({
    ...
    inputSchema: z.object({
      ...
      direction: z.enum(['backward', 'forward']).describe('backward: 从结果找原因；forward: 从原因找结果'),
    ...
    execute: async ({ seed, direction, maxDepth }): Promise<ToolResult> => {
      ...
      const paths =
        direction === 'backward' ? causalGraph.backwardChain(seed, maxDepth) : causalGraph.forwardChain(seed, maxDepth)

      if (paths.length === 0) {
        return toolOk({
          paths: [],
          hint:
            `No causal paths found from '${seed}' in direction '${direction}'. ` +
    ...
    inputSchema: z.object({
      causes: z.array(
        z.object({
          ...
          timelineEvidenceIds: z.array(z.string()).default([]).describe('来自 query_events 的支持该原因的事件 ID'),
          canCoexistWith: z.array(z.string()).default([]).describe('可同时发生的原因标签'),
        })
      ),
    }),
  ...
  const record_event = tool({
    description:
      '如果事件尚未存在，将其记录到 EventStore 中。' + '当你从领域知识或用户描述中发现一个尚未存储的事件时使用此方法。',
    inputSchema: z.object({
    ...
    execute: async ({ id, type, occurredAt, actorId, affectedEntities, payload }): Promise<ToolResult> => {
