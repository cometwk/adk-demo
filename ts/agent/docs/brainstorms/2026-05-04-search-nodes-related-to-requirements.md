...
**架构统一方案（方案 A）**
- R7. 扩展 `PropertySchema`，增加 `agentVisible: boolean` 和 `sensitive?: boolean` 字段，使其能完全描述 `TypeProperty`
- R8. 扩展 `@agentProperty` 装饰器，增加 `agentVisible` 和 `sensitive` 参数
- R9. 实体类成为唯一数据源，Ontology 从 Registry 生成（可选）
- R10. `PropertySchemaConfig.returns` 改名为 `type`，与 `TypeProperty.type` 统一
...
- `search_nodes({ query: '小明' })` 应返回 `xiao_ming`（匹配 name 属性）
- Registry 中 `agentVisible: true` 的属性均参与 query 匹配
...
**Phase 2**
- **依赖**：PropertySchema 扩展（agentVisible、sensitive 字段）
- **文件改动**：registry.ts、decorator.ts、entities.ts
- **可选**：删除 ontology.ts 和 ontology/schema.ts，或从 Registry 生成 Ontology 文档
...
- **架构统一方案 A**：扩展 PropertySchema 增加 agentVisible/sensitive，实体类为唯一数据源。理由：单一数据源避免同步问题，运行时直接可用，改动范围小
- **字段命名统一**：PropertySchemaConfig.returns 改名为 type。理由：与 TypeProperty.type 对齐，语义更清晰

## Dependencies / Assumptions

- Phase 1 已完成：`Graph.searchNodes` 支持 relatedTo 参数
- Phase 2 依赖：PropertySchema 扩展（registry.ts、decorator.ts）
