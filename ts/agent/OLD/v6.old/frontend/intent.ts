...
export function detectIntent(userQuery: string): IntentResult {
	...
	if (scores.size === 0) {
		return { mode: "predictive", intent: "unknown", confidence: 0, matchedKeywords: [] };
	}
	...
	return { mode, intent: bestIntent, confidence, matchedKeywords: bestKeywords };
}
...
export function isDefinitelyDiagnostic(userQuery: string): boolean {
	...
	const outcomeIndicators = [
		"已经发生", "发生了", "already happened", "has occurred", "导致延期",
		"延期了", "失败了", "崩了", "挂了", "出事了", "事故", "复盘",
	];
