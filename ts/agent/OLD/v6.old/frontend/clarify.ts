...
let _questionId = 0;
function qid(): string { return `q_${++_questionId}`; }

// ── Clarify entity ambiguity ──

export function buildEntityClarification(
...
export function resolveTimeWindow(optionId: string): { from: string; to: string } {
	const now = new Date();
	const to = now.toISOString();
	const dayMs = 86400000;

	switch (optionId) {
		case "last_week": return { from: new Date(now.getTime() - 7 * dayMs).toISOString(), to };
		case "last_month": return { from: new Date(now.getTime() - 30 * dayMs).toISOString(), to };
		case "last_quarter": return { from: new Date(now.getTime() - 90 * dayMs).toISOString(), to };
		default: return { from: new Date(now.getTime() - 30 * dayMs).toISOString(), to };
	}
}
...
export function applyIntentClarification(
	...
	switch (optionId) {
		...
		default:
			return { ...current, mode: current.mode ?? "predictive", intent: current.intent ?? "unknown" };
	}
}
