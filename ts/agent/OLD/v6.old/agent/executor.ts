...
function parseModelVerdictPredictive(
	text: string,
	workspace: DecisionWorkspace,
...
function parseModelVerdictDiagnostic(
	text: string,
	workspace: DecisionWorkspace,
): DiagnosticVerdict {
	try {
		...
		return {
			source: "model",
			mode: "diagnostic",
			rankedAttributions: (v.rankedCauses ?? []).map((c: { causeId: string; rationale?: string; citedEvidenceIds?: string[] }) => ({
				causeId: c.causeId,
