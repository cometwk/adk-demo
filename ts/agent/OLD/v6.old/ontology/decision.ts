...
export class DecisionWorkspace {
	...
	addCandidate(label: string, description: string): CandidateAnswer {
		const id = genId("cand");
		const c: CandidateAnswer = { id, label, description, supportingEvidenceIds: [] };
		this.candidates.set(id, c);
		return c;
	}

	getCandidate(id: string): CandidateAnswer | undefined { return this.candidates.get(id); }
	listCandidates(): CandidateAnswer[] { return [...this.candidates.values()]; }
	...
	getCause(id: string): CandidateCause | undefined { return this.causes.get(id); }
	listCauses(): CandidateCause[] { return [...this.causes.values()]; }
	...
	listEvidence(): Evidence[] { return [...this.evidence.values()]; }
	...
	listUncertainties(): Uncertainty[] { return [...this.uncertainties.values()]; }

	// ── Triggered rules ──

	addTriggeredRule(ruleId: string): void { this.triggeredRuleIds.add(ruleId); }
	listTriggeredRules(): string[] { return [...this.triggeredRuleIds]; }
}
