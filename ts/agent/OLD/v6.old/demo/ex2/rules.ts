...
export function registerDbtRules(): void {
	...
	registerRule({
		...
		evaluator(ctx) {
			...
			if (coverage === undefined) return { triggered: false, missingFacts: [{ entityId, property: "testCoverage" }] };
	...
	registerRule({
		...
		evaluator(ctx) {
			...
			if (hasOwner === undefined) return { triggered: false, missingFacts: [{ entityId, property: "hasOwner" }] };
			const triggered = !hasOwner;
			return {
				triggered,
				severity: triggered ? "medium" : "low",
				explanation: triggered
					? `模型 ${entityId} 无负责人，存在治理风险`
					: `模型 ${entityId} 已分配负责人`,
				missingFacts: [],
			};
		},
	...
	registerRule({
		...
		evaluator(ctx) {
			...
			if (count === undefined) return { triggered: false, missingFacts: [{ entityId, property: "downstreamDashboardCount" }] };
