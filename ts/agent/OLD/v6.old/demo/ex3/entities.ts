...
export class Reader extends BaseNode {
	...
	checkBorrowCapacity(_args: Record<string, never> = {}): { canBorrow: boolean; remainingSlots: number } {
		const limit = 3;
	...
	checkOverdueBlock(_args: Record<string, never> = {}): { blocked: boolean; reason: string | null } {
		if (this.hasOverdue) {
...
export class Book extends BaseNode {
	...
	@agentProperty({ returns: "string", description: "图书分类：fiction | nonfiction | reference | textbook" })
	category: "fiction" | "nonfiction" | "reference" | "textbook";

	@agentProperty({ returns: "boolean", description: "是否为新书（上架不到 7 天）" })
	isNew: boolean;
	...
	@agentProperty({ returns: "'available' | 'borrowed' | 'in_library_only'", description: "当前状态" })
	...
	checkNewBookStatus(args: { currentTime: string }): { isNewBook: boolean; daysSinceShelved: number; canCheckout: boolean } {
	...
	checkAvailability(_args: Record<string, never> = {}): { available: boolean; reason: string | null } {
...
export class Library extends BaseNode {
	...
	@agentProperty({ returns: "number", description: "新书限制天数（上架 N 天内不可外借）" })
...
export class BorrowRecord extends BaseNode {
	...
	@agentProperty({ returns: "string", description: "实际归还日期 (ISO format 或 null)" })
	...
	checkOverdue(args: { currentTime: string }): { overdue: boolean; daysOverdue: number } {
