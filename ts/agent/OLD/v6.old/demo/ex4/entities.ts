...
export class Reader extends BaseNode {
	@agentProperty({ returns: "number", description: "当前已借出且未归还的书籍数量" })
	...
	checkBorrowEligibility(_args: Record<string, never> = {}): { eligible: boolean; reason?: string } {
		if (this.currentBorrowCount >= 3) {
			return { eligible: false, reason: `已借 ${this.currentBorrowCount} 本，达到上限 3 本` };
		}
...
export class Book extends BaseNode {
	...
	@agentProperty({ returns: "boolean", description: "是否允许外借（馆员手动标注）" })
	lendable: boolean;
	...
	checkNewBookStatus(
		args: { newBookThresholdDays: number } = { newBookThresholdDays: 7 },
	): { isNew: boolean; daysOnShelf: number; thresholdDays: number } {
		return {
...
export class Library extends BaseNode {
	@agentProperty({ returns: "number", description: "每位读者最多可同时借阅的书籍数量（上限）" })
	maxBorrowPerReader: number;

	@agentProperty({ returns: "number", description: "新书保护期（天）：上架不足此天数不可外借" })
	...
	evaluateBorrowRequest(args: {
		...
		void args;
		return { allowed: false, blockedReasons: ["evaluation delegated to rule engine"] };
	}
