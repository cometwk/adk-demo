---
name: fix-ddl
description: 检查并同步 DDL SQL 与 Go 表结构及测试数据的一致性。当需要新增或修改 DDL（serve/docs/ddl/*.sql）、同步 serve/biz/tables.go 表结构、检查 serve/biz/init.go orm 注册、或修复 serve/docs/data/init.sql 测试数据时使用此 Skill。以 DDL SQL 为准，检查 SQL 语法错误并修复四者之间的差异。
---
...
| 文件 | 职责 |
|------|------|
| `serve/docs/ddl/xxx.sql` | **权威来源**：每张表一个文件，包含建表语句及必要的种子数据 |
| `serve/biz/tables.go` | Go xorm 结构体，需与对应 DDL 字段一一对应 |
| `serve/biz/init.go` | 表注册入口，每张表的 struct 必须在此调用 `orm.MustLoadStructModel[T]()` 注册，并声明对应的 `orm.EntityOps[T]` 变量及 `orm.MustEntityOps[T]()` 初始化 |
| `serve/docs/data/init.sql` | 测试环境初始化脚本，表结构操作需与 DDL 保持同步 |
...
- 读取涉及的 `serve/docs/ddl/<table>.sql`
- 对照 `serve/biz/tables.go` 中对应的 struct
- 检查 `serve/biz/init.go` 中该 struct 是否已完成 orm 注册（见下方步骤 5）
- 检查 `serve/docs/data/init.sql` 中是否有针对该表的 INSERT/DELETE/UPDATE
...
若新增了表（新 struct），需在 `serve/biz/init.go` 中完成以下三步：
