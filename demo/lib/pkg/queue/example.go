...
// 如果提供了 baseCtx，会在 baseCtx 取消时自动停止；否则创建新的 context
func NewQueueExample(baseCtx ...context.Context) (*QueueExample, error) {
	// 初始化数据库连接
	dbDriver := env.MustString("DB_DRIVER")
	dbURL := env.MustString("DB_URL")

	engine, err := orm.NewXormEngine(dbDriver, dbURL)
	if err != nil {
	...
	if err != nil {
		return nil, fmt.Errorf("清理 job_queue 表失败: %w", err)
	}
	...
	// if err != nil {
	// 	return nil, fmt.Errorf("同步表结构失败: %w", err)
	// }

	// 创建队列客户端
	client := NewClient(engine)

	// 如果有基础 context，基于它创建；否则创建新的
	var baseContext context.Context = context.Background()
	if len(baseCtx) > 0 && baseCtx[0] != nil {
		baseContext = baseCtx[0]
	}

	// 创建子 context，当 baseContext 取消时自动取消
	ctx, cancel := context.WithCancel(baseContext)

	// 如果 baseContext 不是 Background，监听其取消并传播
	if baseContext != context.Background() {
		go func() {
			<-baseContext.Done()
			cancel()
		}()
	...
	return &QueueExample{
		...
		consumer: &Consumer{workers: make([]*Worker, 0), ctx: ctx},
		cancel:   cancel,
	}, nil
}
...
func (qe *QueueExample) StartConsumer() {
	...
	for i := 1; i <= 2; i++ {
		...
		// 创建Worker
		worker := NewWorker(qe.client, options, handler)
		qe.consumer.workers = append(qe.consumer.workers, worker)

		// 启动Worker
		workerID := i // 捕获循环变量
		qe.wg.Add(1)
		go func(wID int, w *Worker) {
			defer qe.wg.Done()
...
func (qe *QueueExample) GetQueueStats(ctx context.Context) (map[string]any, error) {
	...
	for _, queue := range queues {
		count, err := engine.Context(ctx).Where("queue_name = ?", queue).Count(new(JobQueue))
		if err != nil {
			return nil, fmt.Errorf("查询队列 %s 统计失败: %w", queue, err)
		}
		queueStats[queue] = count
	}

	// 获取总任务数
	totalJobs, err := engine.Context(ctx).Count(new(JobQueue))
	if err != nil {
		return nil, fmt.Errorf("查询总任务数失败: %w", err)
	}

	// 获取锁定任务数
	lockedJobs, err := engine.Context(ctx).Where("lease_until IS NOT NULL").Count(new(JobQueue))
	if err != nil {
		return nil, fmt.Errorf("查询锁定任务数失败: %w", err)
	}

	// 获取历史任务统计
	completedJobs, err := engine.Context(ctx).Where("status_final = ?", "completed").Count(new(JobHistory))
	if err != nil {
		return nil, fmt.Errorf("查询完成任务数失败: %w", err)
	}

	deadLetterJobs, err := engine.Context(ctx).Where("status_final = ?", "dead_letter").Count(new(JobHistory))
	if err != nil {
...
func RunExample(ctx ...context.Context) {
	...
	// 创建示例实例，传入 context（如果有）
	var exampleCtx context.Context
	...
	example, err := NewQueueExample(exampleCtx)
	if err != nil {
