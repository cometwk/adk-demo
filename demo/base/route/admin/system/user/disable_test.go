...
func TestDisable(t *testing.T) {
	e, _ := setup()
	t.Run("DisableSuccess", func(t *testing.T) {
		...
		p := `{"uuid":"test-uuid"}`
		rec := testutil.Post(e, "/admin/system/user/disable", p)
		assert.Equal(t, http.StatusOK, rec.Code)

		// 验证用户已被禁用
		user, err := model.UserModel.GetOne("test-uuid")
		assert.NoError(t, err)
		assert.True(t, user.Disabled, "用户应该被禁用")
		assert.Equal(t, "测试用户", user.Name)
		assert.Equal(t, "testuser", user.UserId)
	})

	t.Run("DisableMissingUUID", func(t *testing.T) {
		p := `{}`
		rec := testutil.Post(e, "/admin/system/user/disable", p)
