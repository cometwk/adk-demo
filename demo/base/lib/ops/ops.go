...
func Recorder(next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		// 登录接口不验证
		if strings.HasPrefix(c.Request().URL.Path, "/api/v1/login") {
			return next(c)
		}
		if strings.HasPrefix(c.Request().URL.Path, "/api/v1/pub") {
			return next(c)
		}
...
func opsReadBody(cc ctx.Context) (string, string, bool) {
	...
	if strings.Contains(content_type, "x-www-form-urlencoded") {
