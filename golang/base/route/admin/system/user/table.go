...
import (
	...
	"github.com/lucky-byte/base/pkg/secure"
...
func (h *handler) searchPage0(c echo.Context, params map[string]string) (any, error) {
	...
	type UserAcl struct {
		...
		AgentName string `xorm:"agent_name" json:"agent_name"` // 代理商名称 这里跟包的划分产生了冲突, 先这样
		MerchName string `xorm:"merch_name" json:"merch_name"` // 商户名称
	}

	var rows []UserAcl
	count, err := session.Table("users").Select("users.*, acl.name as acl_name, acl.code as acl_code, agent.name as agent_name").
		Join("LEFT", "acl", "acl.uuid = users.acl").
		Join("LEFT", "agent", "agent.agent_no = users.bind_no").
		Join("LEFT", "merch", "merch.merch_no = users.bind_no").
