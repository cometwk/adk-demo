...
type handler struct {
	*serve.CrudHandler[queue.JobQueue]
}

func jobAttach(e *echo.Group) {
	handler := &handler{
		serve.NewCrudHandler[queue.JobQueue](""),
	}
...
func (h *handler) Update(c echo.Context) error {
	...
	// 采用map方式能识别字段
	// user := util.StructToMapFast(row)
	// _, err := h.Ops.WithSession(session).UpdateByPK(user)
...
// 需要联表操作
func (h *handler) SearchPagex(c echo.Context) error {

	q := c.QueryParams()
	params := urlValuesToMap(q)

	pageResult, err := h.searchPage0(c, params)
	if err != nil {
		return err
	}
