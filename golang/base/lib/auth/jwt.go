...
// 生成 JWT
func JWTGenerate(c echo.Context, j *AuthJWT) (string, error) {
	keys, err := getSignKeyPair(c)
	if err != nil {
		return "", err
	}
...
// 解析 JWT
func JWTParse(c echo.Context, tokenStr string) (*AuthJWT, error) {
	keys, err := getSignKeyPair(c)
	if err != nil {
		return nil, err
	}
...
// 从数据库获取签名密钥
func getSignKeyPair(c echo.Context) (*keypair, error) {
	ql := `select jwtsignkey, jwtsignkey2 from account limit 1`
	var keys keypair

	if err := db.SelectOneX(c, ql, &keys); err != nil {
