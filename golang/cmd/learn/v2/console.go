...
func NewLauncher() *consoleLauncher {
	defaultStreamingMode := string(agent.StreamingModeNone)
	config := &consoleConfig{
		streamingMode:       agent.StreamingMode(defaultStreamingMode),
		streamingModeString: defaultStreamingMode,
	}

	fs := flag.NewFlagSet("console", flag.ContinueOnError)
	fs.StringVar(&config.streamingModeString, "streaming_mode", defaultStreamingMode,
...
func (l *consoleLauncher) Run(ctx context.Context, config *launcher.Config) error {
	...
	for {
		...
		if firstMessage {
			parts := []*genai.Part{
				genai.NewPartFromText(INITIAL_REMINDER),
				genai.NewPartFromText(userInput),
			}
			userMsg = genai.NewContentFromParts(parts, genai.RoleUser)
			firstMessage = false
		} else if l.roundsWithoutTodo > 10 {
			parts := []*genai.Part{
				genai.NewPartFromText(NAG_REMINDER),
				genai.NewPartFromText(userInput),
			}
			userMsg = genai.NewContentFromParts(parts, genai.RoleUser)
		}
			...
			if err != nil {
				...
				for _, p := range event.LLMResponse.Content.Parts {
					...
					if p.FunctionCall != nil {
						...
						if p.FunctionCall.Name == "TodoWrite" {
